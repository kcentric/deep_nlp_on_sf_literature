...
# NOTE to self: The DATAFRAME-making method is working too slow. How to fasten it?
# TODO: rigorously test each of these methods once?
...
class CorpusProcessor:
    ...
    def __init__(self, filepath=FILEPATH, cleaning_method="Lem", measure_processing_time=True,
                 corpus_contains_mostly_stories=True):  # For user convenience, default to measuring
                                                        # processing time.
        self.dataframe = None  # An attribute to hold a Pandas dataframe of sentences and cleaned sentences from our text
        self.text_as_a_wordlist = []  # An attribute to make further processing convenient AFTER the corpus we're working with has
                                      # been turned into a list of words once.
        self.corpus_as_a_booklist = []  # An attribute that'll usually be filled after the previous one is; from having the text as
        ...
        # Run the basics. Note: The create_dataframe method does a lot of essential set-up for the other methods by virtue of
        # executing first.
    ...
    def process(self, filepath, cleaning_method, corpus_contains_mostly_stories=True):
        """Perform initial cleaning and create the dataframe."""
        with open(filepath, "r") as file:
            original_text = file.read().lower()
            self.dataframe = self.create_dataframe(original_text, cleaning_method,
    ...
    def create_dataframe(self, text, cleaning_method, find_pos=False, corpus_contains_mostly_stories=False):
        ...
        self.text_as_a_wordlist = list(itertools.chain.from_iterable(list_of_lists))  # Now we can easily use this wordlist for further processing
    ...
    def write_dataframe_to_csv(self):
        with open("../../sci-fi_text_cleaned.csv", "w+") as our_file:
    ...
    def test(self, filepath="small_sample_text_for_testing.txt", cleaning_method="Lem", find_pos=False):
        ...
        cleaned_text_as_df = self.create_dataframe(test_text, cleaning_method=cleaning_method,
                                                   find_pos=find_pos)
