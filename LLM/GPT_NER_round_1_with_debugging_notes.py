# A during-process created version of its namesake file created to preserve a couple debugging TODOs.
#  Illustrates how using GitHub could be very helpful because all my commits could be preserved there
#  without making so much effort.
#  If this project was hosted on GitHub, I could have started an issue and resolved it so anyone could
#  know all the steps. But not right now.
...
# TODO: Fix. There is some error in the code, where entities that aren't labeled somehow get labels.
#  plug gets a NONE label and ging gets a TECHNOLOGY label. These tokens are present in the tokenized
#  SENTENCE but NOT present in the sentence's entities. So how do they get anything except NONE?
#  is it somehow for "ging" that it has an "in" which IS labeled as TECHNOLOGY because "in" is a part
#  "lead-in cord"?
#  NOTE!!: FIXED in OG file. How?! By inserting 'if any(t == token or t[1:] == token for t in tokens_of_entity)'
#  instead of 'if any(t in token for t in tokens_of_entity)'.
    ...
    for key in super_dict:
        ...
        if label_the_unlabeled:
            ...
            for token in tokenized_sentence:
                ...
                print(tokenized_entities)  # Just to check if "in" is there?
                for ent_tuple in tokenized_entities:
                    tokens_of_entity = ent_tuple[0]  # The list containing the tokens of the named entity
                    # Use an 'in' statement rather than an "==" statement to account for the fact
                    #  that there might be a 'Ġ' in the sentence-token which isn't there in the
                    #  entity-token.
                    if any(t in token for t in tokens_of_entity):  # If for any t, t is in token, get True
...
if __name__ == "__main__":
    ...
    print(len(ready_for_dataframing))
    for i in range(30):
        print(ready_for_dataframing[i])

    # print(tokenize_sents_or_strings(all_sents)[:30])
    # print(convert_data_to_dataframe(""))

    # NOTE: I was wondering if there could be a possibility where an entity-tokenization has a 'Ġ'
    #        while its corresponding sentence-token doesn't have a 'Ġ'. If you have the same doubt,
    #        this can be tested by running the 'list_of_dicts_to_list_of_lists' function and then
